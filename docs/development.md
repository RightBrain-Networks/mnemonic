...
The inner plugin manifest is `0.38.0`. Before release, parse the marketplace
and inner plugin manifests, then exercise a disposable fresh `0.38.0` install
plus a `0.18.0 -> 0.38.0` marketplace/plugin update. Use an
...
Current application/API/MCP/dashboard versions are `0.60.0`, plugin is `0.38.0`,
and Alembic head is `0041_artifact_passages`. Validate all surfaces
together with the existing regression suites. This release adds token-bounded
semantic artifact passages and explicit multi-project discovery. Verify current
revision/hash/model/config publication fences, tokenizer limits, resumable jobs,
sensitive exclusion, rebuild/restore behavior, one global ranked page, per-project
coverage, and shared capacity bounds. Preserve the previous query intent, evidence,
field/date/tag scopes, diagnostics, and semantic disposition contracts. API, worker,
MCP, dashboard and plugin must upgrade together with migration
`0041_artifact_passages`; no new configuration variable is required. It retains
