"""Bounded, typed code-review contracts independent of the work object graph."""
...
RepositoryKey = Annotated[
    str,
    StringConstraints(
        strict=True, min_length=1, max_length=80, pattern=r"^[A-Za-z0-9][A-Za-z0-9._-]*$"
    ),
]
FullCommit = Annotated[
    str, StringConstraints(strict=True, pattern=r"^(?:[0-9a-f]{40}|[0-9a-f]{64})$")
]
...
class ReviewModel(BaseModel):
    model_config = ConfigDict(
        extra="forbid", from_attributes=True, hide_input_in_errors=True, allow_inf_nan=False
    )
    ...
    def _locators(self) -> None:
        ...
        if self.repository_url is not None:
            ...
            if (
                ...
                or parts.query
                or parts.fragment
                or any(c.isspace() for c in self.repository_url)
            ):
...
class CodeReviewScopeInput(ReviewModel):
    ...
    def scope_is_unique_and_bounded(self) -> Self:
        ...
        identities = {
            (r.repository_url, r.checkout_path, r.base_commit, r.head_commit)
...
CodeReviewHandoffArgument = Annotated[
    CodeReviewHandoffInput | SkipJsonSchema[None],
    BeforeValidator(reject_null_review_argument),
]
...
class ReviewPolicyRead(ReviewTimestamp):
    ...
    def decision_corresponds(self) -> Self:
        if self.decision != review_policy(
            self.priority_at_closeout,
            self.required_min_priority,
            self.optional_min_priority,
            self.allow_remediation_code_reviews,
            self.remediation_depth,
        ):
...
class CodeReviewContext(ReviewModel):
    ...
    def provenance_corresponds(self) -> Self:
        ...
        if (
            self.remediation_origin is not None
            and self.remediation_origin.depth != self.remediation_depth
        ):
...
class ReviewQueueRow(ReviewTimestamp):
    ...
    def queue_row_corresponds(self) -> Self:
        ...
        if self.lease is not None:
            from .schemas import LeasePublic

            lease = LeasePublic.model_validate(self.lease)
            if (
                not review
                or self.state != "requested"
                or (lease.purpose != "code_review" or lease.code_review_id != self.id)
            ):
                raise ValueError("Queue lease must belong to this requested review")
        if self.review_available and (
            not review
            or self.state != "requested"
            or self.work_status != "done"
...
def review_policy(
    priority: int, required: int, optional: int, allow: bool, depth: int
) -> ReviewDecision:
    if depth == 2:
