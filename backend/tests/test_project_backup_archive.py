...
@pytest.mark.parametrize("consume_before_snapshot", [False, True])
def test_restore_never_revives_sensitive_approval_tokens(
    api, project, postgres_engine, tmp_path, consume_before_snapshot,
):
    api.app.state.artifact_storage = ArtifactStorage(tmp_path / "artifacts", max_bytes=1024)
    artifact = upload(api, project, metadata={"sensitive": True})
    path = f"/api/v1/projects/{project['id']}/artifacts/{artifact['id']}/content"
    challenge = api.get(path)
    assert challenge.status_code == 428, challenge.text
    token = challenge.json()["detail"]["context"]["approval_token"]
    approved = {"X-Artifact-Metadata": json.dumps({
        "approval_token": token, "human_approved": True,
    })}
    if consume_before_snapshot:
        assert api.get(path, headers=approved).status_code == 200
    content = _export(postgres_engine, project)
    original = _snapshot(postgres_engine, project)
    assert token.encode() not in bz2.decompress(content)
    if not consume_before_snapshot:
        assert api.get(path, headers=approved).status_code == 200
    other = api.post("/api/v1/projects", json={"name": "Other approval"}).json()
    other_artifact = upload(api, other, metadata={"sensitive": True})
    other_path = f"/api/v1/projects/{other['id']}/artifacts/{other_artifact['id']}/content"
    other_token = api.get(other_path).json()["detail"]["context"]["approval_token"]
    untouched = _snapshot(postgres_engine, other)

    restore_project(postgres_engine, UUID(project["id"]), io.BytesIO(content))

    restored = _snapshot(postgres_engine, project)
    restored_approval = restored["artifact_access_approvals"][0]
    original_approval = original["artifact_access_approvals"][0]
    assert restored_approval["consumed_at"] is not None
    assert {key: value for key, value in restored_approval.items() if key != "consumed_at"} == {
        key: value for key, value in original_approval.items() if key != "consumed_at"
    }
    if consume_before_snapshot:
        assert restored_approval["consumed_at"] == original_approval["consumed_at"]
    assert restored["artifact_audit"] == original["artifact_audit"]
    assert _snapshot(postgres_engine, other) == untouched
    denied = api.get(path, headers=approved)
    assert denied.status_code == 428, denied.text
    new_token = denied.json()["detail"]["context"]["approval_token"]
    assert new_token != token
    assert api.get(path, headers={"X-Artifact-Metadata": json.dumps({
        "approval_token": new_token, "human_approved": True,
    })}).status_code == 200
