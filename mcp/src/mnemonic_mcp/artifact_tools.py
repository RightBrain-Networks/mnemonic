...
def _register_reads(server: FastMCP, api: MnemonicAPI) -> None:
    ...
    async def list_artifacts(
        ...
        return cast(ArtifactPage[ArtifactRead], await api.request(
    ...
    async def get_artifact(project_id: UUID, artifact_id: UUID) -> ArtifactRead:
        ...
        return await _get_artifact(api, project_id, artifact_id)
    ...
    async def list_artifact_history(
        ...
        if q is not None:
            params["q"] = q
        return cast(ArtifactHistory, await api.request(
    ...
    @server.tool(annotations=_READ)
    async def download_artifact(project_id: UUID, artifact_id: UUID) -> ArtifactDownload:
        ...
        return ArtifactDownload(artifact=artifact, content_base64=base64.b64encode(content).decode())
