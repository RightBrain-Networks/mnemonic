...
def _register_claim_tools(server: FastMCP, api: MnemonicAPI) -> None:
    ...
    async def claim_work(
        ...
        """Claims use claim_request_id as the retry key; client_operation_id is not accepted. Supply holder_client and holder_session_id for your own client/session. First call get_work(status_only=true) for current status and project lease_settings. Request lease_minutes=default_minutes for initial session startup and investigation; choose later durations from estimated remaining session work within minimum_minutes and maximum_minutes. Any in-range whole-minute request is accepted; omission uses the project default. Preserve the exact lease_minutes argument, including omission, across uncertain claim retries. Explicitly supply session_transcript={client, path} using an absolute path visible to the Mnemonic backend, or null when unavailable. Claude Code uses client=claude_code; its JSON/JSONL format is auto-detected. Retain this assertion unchanged on claim retries. Acquire an expiring exclusive lease for already-authorized work. Implementation requires pending work; deferred work needs explicit human direction before moving to pending. Code review instead claims the original Done item with purpose=code_review, exact code_review_id and mode=cold|warm; do not reopen it. For a manual review with no scope, first establish the range from retained context and supply code_review_handoff with mode=warm; preserve it on exact retries. Later claims omit handoff. This minimal response contains coordination only, never context/handoff. Cold attempts must use this tool, never claim_and_recall or contextual reads before findings freeze. holder_client names the actual client; holder_session_id is this independent agent's native session ID or one generated-and-retained Mnemonic session UUID. If compaction lost the token and exact original claim arguments are unavailable, read get_work(status_only=true) and confirm no other active session is working on this item before using force=true with a new claim_request_id. Force replaces the active lease, invalidates its old token, and grants no extra authority. Keep force and all arguments unchanged on uncertain retries; a released or replaced force request cannot reclaim again. Keep lease_token in private active-session state, never checkpoints/logs/chat. Identical active requests replay without extending expiry; capability recovery grants no new authority. Human gates still prohibit implementation. After unknown outcome retry promptly with exactly the same claim_request_id and arguments."""
