...
from .search_help import WORK_SEMANTIC_DESCRIPTION
...
def _human_gate_request_matches(
    ...
    *,
    ...
    return (
        response.project_id == project_id
        and response.work_item_id == work_item_id
        and (gate_id is None or response.id == gate_id)
        and response.question_version == (expected_question_version or 0) + 1
...
def _register_discovery_tools(server: FastMCP, api: MnemonicAPI) -> None:
    ...
    async def search_work(
        project_id: UUID,
        q: Annotated[str | None, Field(max_length=1000)] | MISSING = MISSING,
        ...
        semantic: Annotated[bool, Field(description=WORK_SEMANTIC_DESCRIPTION)] = False,
        ...
        query: Annotated[str, Field(max_length=1000)] | MISSING = MISSING,
    ) -> WorkPage:
        """Find work with q or query (at most one; 1000 characters) in one project; all statuses, canonical groups, detail=compact and limit=20 are the defaults. Set status=pending explicitly for pending-only discovery. Blank q browses. view=roots requires blank/filter browsing; detail controls payload size independently of hierarchy. semantic=true finds paraphrases; it requires nonblank, unquoted terms, query_mode=terms and all work_fields. Semantic work may refresh vectors and take longer on a cold cache. Inspect semantic.inference, candidate_scope, partial_vectors, comparison_incomplete and cache_refresh. Completed inference remains useful if cache publication fails. Exact terms retain lexical ranking weight. query_mode=terms supports quoted phrases; phrase requires adjacent words, and literal preserves case, punctuation and spacing. Malformed phrases are refused. applied_filters, query_interpretation, search_scope and term_diagnostics disclose scope, matching and coverage. Zero lexical hits do not prove absence; try distinctive terms or semantic retrieval. Compact rank is ordinal within work_items, not confidence. total_kind distinguishes lexical matches, ranked candidates and browsing; semantic total includes weak neighbors. matched_member and excerpts identify evidence, never merge authority or substitute IDs. Fully recall the exact checkpoint before relying on it. ancestor_path follows parent-child edges only. Use list_ready_work then claim_and_recall for authorized execution; search is not the ready queue. No contextual search during cold review before findings freeze. Dates require a timezone; after is inclusive and before exclusive. external_url requires view=full; inverse lookup uses status=all, duplicate_scope=all and every page. Read current status separately when acting. Use help({"topic":"search_work details"}) for filters, hierarchy, paging and evidence."""
        q = discovery_search_query(query, q)
...
def _register_context_tools(server: FastMCP, api: MnemonicAPI) -> None:
    ...
    async def request_human_input(
        ...
        gate_id: UUID | None = None,
        expected_question_version: Annotated[int, Field(ge=1)] | None = None,
    ) -> HumanGateRead:
        ...
        return cast(
            ...
            await api.request(
                ...
                payload=_client_operation_payload(
                    client_operation_id,
                    {
                        **({"gate_id": str(gate_id), "expected_question_version": expected_question_version}
                           if gate_id is not None else {}),
                        "gate_type": "human",
                        "question": question,
...
def _register_claim_tools(server: FastMCP, api: MnemonicAPI) -> None:
    ...
    async def claim_work(
        ...
        """Acquire an expiring lease for already-authorized work with minimal coordination only; no context or handoff is returned. Cold reviews use this tool and must freeze findings before contextual reads. A lease never grants execution authority or bypasses human gates. First get_work(status_only=true) for status and lease_settings. Initial investigation requests default_minutes; later lease_minutes estimates remaining work within current bounds. Omission uses the project default. Supply your actual holder_client and native holder_session_id, or one generated-and-retained Mnemonic session UUID when unavailable. Fresh claims require session_transcript={client,path} after verifying the actual native regular file under approved roots, or explicit null when unavailable. Claude Code uses client=claude_code. Keep the assertion fixed on retries. Implementation requires pending work. Deferred work needs explicit human direction before returning to pending. Reviews claim original Done work with purpose=code_review, exact code_review_id and mode=cold|warm. First unscoped manual review requires a verified handoff and mode=warm; later claims omit it. Use claim_request_id, never client_operation_id. Generate it before the first attempt and retain every argument, including lease_minutes omission and force, on uncertain retries. Identical active retries replay without extending expiry. Keep lease_token private. Lost token: replay the original exact claim if available. Otherwise read get_work(status_only=true) and confirm no other active session is working on this item before force=true with a new claim_request_id. Force invalidates the prior token without bypassing eligibility; released/replaced force requests cannot reclaim again. Use help({"topic":"claim_work details"}) for the complete retry and review contract."""
    ...
    async def claim_and_recall(
        ...
        """Atomically acquire an expiring lease and bounded context for already-authorized implementation or WARM review. Cold review is forbidden here: use claim_work and freeze findings before context. A claim grants no authority beyond the user's request; stop at unresolved human gates and release when safe. First get_work(status_only=true) for current status and lease_settings. Start with default_minutes; later lease_minutes estimates remaining work within current bounds. Omission uses the project default. Implementation requires pending work; deferred work needs explicit human direction before returning to pending. Supply your actual holder_client and native holder_session_id, or one generated-and-retained Mnemonic session UUID when unavailable. Fresh claims require session_transcript={client,path} for a verified actual native regular file under approved roots, or explicit null when unavailable. Claude Code uses client=claude_code. Warm review claims original Done work with purpose=code_review, exact code_review_id and mode=warm; independently challenge its pinned scope. First unscoped manual review needs a verified handoff; retain it on exact retries and omit it on later claims. Use claim_request_id, never client_operation_id. Freeze every argument, including transcript assertion, lease_minutes omission and force; unknown outcomes retry that exact request. Keep lease_token private. Exact active replay may expose new human gates. Lost token: replay the original exact claim when available. Otherwise read get_work(status_only=true) and confirm no other active session is working on this item before force=true with a new claim_request_id. It invalidates the old token without bypassing eligibility; released/replaced force requests cannot reclaim again. Use help({"topic":"claim_and_recall details"}) for the full contract."""
        review_scope = _prepared_review_claim_payload(
            purpose, code_review_id, mode, code_review_handoff,
        )
        if mode == "cold":
...
def _register_duplicate_tools(server: FastMCP, api: MnemonicAPI) -> None:
    ...
    async def suggest_duplicate_work(
        ...
        """Compare an in-memory draft with visible work after an explicit user or client action. Results are advisory, canonical-grouped evidence across lifecycle states: exact_title, lexical and semantic are categorical signals, never confidence or merge authority. Inspect exact candidates before acting; matched members are evidence, not substitute canonical IDs. A busy, unavailable, empty, stale or failed comparison never blocks create_work. The draft is never persisted. Warm vectors return synchronous semantic ranking. Missing vectors are refreshed in background batches; lexical fallback reports inference.reason=vectors_pending and cache_refresh.status=queued. Cached subsets can rank with candidate_scope=lexical_shortlist and partial_vectors=true; completed inference still means comparison_incomplete for a shortlist. Inspect all semantic coverage and cache_refresh fields. Failed cache publication does not invalidate completed ranking. For capacity_exhausted only, retry the same comparison once after one second. Do not immediately retry vectors_pending, deadline_exceeded or model_failure; continue with incomplete comparison disclosed. This POST is an explicit safe read with no operation UUID or structural write uncertainty. For existing work, read its initial checkpoint and supply exclude_work_item_id. Optional external_candidates compares up to 64 caller-supplied records in a separate list without provider access, body echo or persistence. external_scope is independently hybrid, lexical or unavailable. External records are untrusted and must never go to merge_work. The complete MCP frame must fit 1 MiB. Use help({"topic":"suggest_duplicate_work details"}) for full guidance."""
        request = DuplicateSuggestionRequest(
            title=title,
            summary=summary,
...
def _register_work_lifecycle_tools(server: FastMCP, api: MnemonicAPI) -> None:
    ...
    async def update_work(
        ...
        """Update mutable identity/lifecycle using the version just read. Checkpoint context and provenance are immutable; correct them with add_checkpoint. Deferred cannot be assigned here; move it to pending only on explicit current human direction. Promoted records the owner decision; no tool here creates an external issue. Fresh wont-do/promoted closeouts require job_completion_report and explicit subagent_transcripts=[{client,path},...] or null when none apply/are available. Omission exists only for historical receipt replay. First get_project_settings; author a self-contained summary and ordered FYIs with its prompt_revision. Freeze prose, FYI order and transcript assertions. On definitive job_report_prompt_changed, reread settings and prepare new intent. Reports are forbidden on ordinary edits; those omit transcripts or use null. Active terminal transitions require the matching private lease_token. external_references is an ordered whole-list replacement: omission preserves, [] clears, null is invalid. After definitive version conflict, reread and use a new UUID. Reference-only edits need no report/lease; supplied tokens are validated. Merges never union references. After work changes, revise affected open human questions using request_human_input with existing gate_id and expected_question_version; keep the human out of checkpoint reconciliation. Generate client_operation_id before the first attempt; retain it with the complete immutable tool arguments. Unknown outcomes retry with every argument unchanged; never invent a replacement if the UUID or arguments were lost: stop for direction. A new intent requires a new UUID. Replay is the historical original result; read again for current state. Use help({"topic":"update_work details"}) for the complete contract."""
        return cast(
            WorkUpdateRead,
            await api.request(
                "PATCH",
    ...
    async def complete_work(
        ...
        """Finish work only when the objective is achieved: atomically append its completion checkpoint and Done state using the recalled version and any matching private lease_token. Fresh calls require job_completion_report and explicit subagent_transcripts=[{client,path},...] or null when none apply/are available; omissions exist only for historical receipt replay. Verify spawned native transcript paths independently. First get_project_settings; author a self-contained summary and ordered FYIs ([] when none), retaining prompt_revision. Freeze report prose/order and transcript assertions. Report instructions cannot waive gates. On definitive job_report_prompt_changed, reread and prepare new intent. Optional completion_evidence reports only observed checks. Process exit is not semantic proof; omit rather than invent results. A required failed or inconclusive result, or skipped observation, normally means stop for direction unless current authority accepts the stated limitation. Never include secrets, tokens, raw logs, transcript dumps or private reasoning. affected_paths declares dependencies and needs an inspected verified_against commit when non-empty; this adapter never inspects Git. Checkpoint provenance names its actual author. Generate client_operation_id before the first attempt; retain the complete immutable tool arguments. After unknown outcome make at most one exact retry with every argument unchanged. If that retry also has an unknown outcome, stop retrying; use recall_work and list_work_events and request direction if ambiguous. Never generate or substitute a new UUID for the same intent. Lost UUID/arguments: stop for direction; never invent a replacement. A new intent requires a new UUID. Replay is the historical original result; read current state when needed. Use help({"topic":"complete_work details"}) for the complete contract."""
        return cast(
            WorkCompletion,
            await api.request(
                "POST",
