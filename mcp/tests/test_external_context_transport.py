...
async def test_maximum_context_fits_both_sdk_representations(
    settings, work_context, adjacent_relationship, progress_event, human_gate, resolved_human_gate,
):
    context = maximum_external_context(work_context, adjacent_relationship, progress_event,
    ...
    assert all(len(structured[name+"_relationships"]) == 100
