...
def maximum_external_context(context, adjacent, event, gate, resolved_gate):
    ...
    refs = [{
        ...
        "kind": "tracked-by", "state": "unknown", "label": "😀" * 120,
    ...
    checkpoint.update(prompt="a" + "\x01" * 99999,
    ...
    for index, direction in enumerate(("incoming", "outgoing", "undirected")):
        ...
        for i in range(100):
            ...
            edge["relationship"].update(id=str(UUID(int=3000+index*100+i)),
    ...
    result["relationship_counts"] = {"incoming": 100, "outgoing": 100, "undirected": 100,
                                     "total": 300}
    result["duplicate_merge_eligibility"].update(incident_blocks_count=200, has_unresolved_gate=True)
    result["readiness"].update(unresolved_blocker_count=100, is_blocked=True, is_ready=False,
                               unresolved_gate_count=20, is_gated=True, display_state="waiting")
    result["recent_events"] = [{
        **deepcopy(event), "id": i+1, "event_type": "work_updated", "body": None,
        "metadata": {"work_version": 3, "changes": {
    ...
    for resolved, name, template in ((False, "unresolved_gates", gate),
                                      (True, "recent_resolved_gates", resolved_gate)):
        result[name] = []
        for i in range(20):
            item = deepcopy(template)
            item.update(id=str(UUID(int=4000+int(resolved)*100+i)),
                        question="a"+"\x01"*3999,
                        resolution="a"+"\x01"*3999 if resolved else None)
            for revision_name in ("requested_context_revision", "current_context_revision",
