"""Static-only fixtures: MCP completion-evidence models must remain closed to extras."""
...
CommandVerificationInput(
    ...
    command="uv run ty check src/mnemonic_mcp",
    exit_code=0,
...
CompletionEvidenceEpisodeRead(
    ...
    completion_checkpoint=cast(CheckpointPointer, object()),
    verification_results=[],
    artifact_references=[],
    unexpected_completion_evidence_field=True,  # ty: ignore[unknown-argument]
)
