...
def _assert_installed_details(executable: str, config_directory: Path) -> None:
    ...
    assert "Mnemonic (mnemonic) 0.42.1" in output
...
def _assert_installed_component_inventory(root: Path) -> None:
    ...
    assert manifest["version"] == "0.42.1"
    assert {
...
def test_authentic_sequential_and_fresh_plugin_installs_are_exact(tmp_path: Path):
    ...
    assert synthetic_manifest["version"] == "0.42.1"
    ...
    upgraded_root = _active_plugin_root(executable, sequential_config, "0.42.1")
    ...
    fresh_root = _active_plugin_root(executable, fresh_config, "0.42.1")
