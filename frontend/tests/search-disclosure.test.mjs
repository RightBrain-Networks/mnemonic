...
test("hierarchy search requires disclosure while child hierarchy listing keeps its existing shape", () => {
  ...
  assert.deepEqual(decodeHierarchySearchPage({ ...page, detail: "full", work_rank_scope: "work_items", ...metadata }, project, 20, 0, { status: "pending" }), { ...page, detail: "full", work_rank_scope: "work_items", ...metadata });
  assert.throws(() => decodeHierarchySearchPage({ ...page, detail: "full", work_rank_scope: "work_items", ...metadata }, project, 20, 0, { status: "done" }));
});
