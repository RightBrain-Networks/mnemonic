...
test("work search includes the explicit full view and defaults to lexical retrieval", () => {
  ...
  assert.equal(params.toString(), "status=pending&sort=created&detail=full&view=full&limit=20&offset=0&duplicate_scope=canonical&q=database+migration");
...
test("semantic work search trims and requires a nonblank query", () => {
  ...
  assert.equal(blank.toString(), "status=all&sort=updated&detail=full&view=roots&limit=20&offset=0&duplicate_scope=canonical");
});
...
test("ordinary browse pages structural roots and child pages inherit filters", () => {
  ...
  assert.equal(browse.toString(), "status=pending&sort=updated&detail=full&view=roots&limit=20&offset=20&duplicate_scope=canonical");
  const active = workSearchParams({ status: "active", sort: "priority", limit: 20, offset: 0, query: "" });
  assert.equal(active.toString(), "status=active&sort=priority&detail=full&view=roots&limit=20&offset=0&duplicate_scope=canonical");
...
test("duplicate audit scopes force flat pages and bind group filters only to audit views", () => {
  assert.equal(
    ...
    "status=all&sort=updated&detail=full&view=full&limit=20&offset=0&duplicate_scope=aliases"
  );
  assert.equal(
    workSearchParams({
      status: "all",
    ...
    "status=all&sort=updated&detail=full&view=full&limit=20&offset=0&duplicate_scope=all&canonical_work_item_id=7a5dc555-0a6d-4f92-9678-1647524827c8"
  );
