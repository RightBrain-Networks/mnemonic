...
for (const scenario of [
  { reason: "deadline_exceeded", cacheStatus: "not_needed",
    notice: "Comparison incomplete: semantic matching was unavailable.",
    advice: "An immediate retry is not recommended." },
  { reason: "vectors_pending", cacheStatus: "queued",
    notice: "Comparison incomplete: semantic vectors are being prepared in the background.",
    advice: "continue your work without an immediate retry." }
] as const) {
  test(`incomplete duplicate comparison (${scenario.reason}) stays visible and creation remains available`, async ({ page }, testInfo) => {
    ...
    await page.route("**/api/mnemonic/projects/*/duplicate-suggestions", async (route) => {
      ...
      await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({
        ...
        semantic: { inference: { status: "unavailable", reason: scenario.reason }, candidate_scope: "none", partial_vectors: false,
          comparison_incomplete: true, retry: null, cache_refresh: { status: scenario.cacheStatus, reason: null } }
      }) });
    });
    await page.goto("/work-items");
    await page.locator("#project-select").selectOption(state.projectId);
    ...
    const notice = dialog.getByText(scenario.notice, { exact: false });
    await expect(notice).toBeVisible();
    await expect(notice).toContainText(scenario.advice);
    await expect(dialog.getByRole("button", { name: "Create work and checkpoint" })).toBeEnabled();
    await notice.scrollIntoViewIfNeeded();
    await page.screenshot({ path: testInfo.outputPath(`incomplete-comparison-${scenario.reason}.png`), fullPage: true });
  });
}
