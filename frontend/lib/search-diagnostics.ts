...
export function decodeTermDiagnostics(value: unknown, total: number, searched: Source[]): TermDiagnostic[] {
  if (!Array.isArray(value) || value.length > 500 || total > 0 && value.length > 0) {
  ...
  return value.map((entry) => {
    ...
    if (!row || !exactKeys(row, ["term", "matches"]) || !boundedText(row.term, 1000)
