...
import { ARTIFACT_DISABLED_MESSAGE, artifactLibraryPath, artifactLocation, artifactPath, decodeArtifactLimitError, decodeArtifactPage, fetchArtifactStatus, formatArtifactSize, type Artifact, type ArtifactPage, type ArtifactSort, type ArtifactStatus } from "@/lib/artifacts";
...
export default function ArtifactLibrary({ projectId, maximumBytes, refreshSignal, onPendingChange }: {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const response = await fetch(`${artifactPath(projectId)}?${query}`, { cache: "no-store", signal: controller.signal });
        ...
        setPage(decodeArtifactPage(result, projectId));
      } catch (error) {
        if (!controller.signal.aborted) { if (!checkedStatus) recordStatus(null); setPage(null); setLoadError(errorMessage(error)); }
      } finally { if (!controller.signal.aborted) setLoading(false); }
    }
    void load();
    return () => controller.abort();
  }, [projectId, search, sort, order, offset, includeDeleted, refresh, refreshSignal, workFilter, maximumBytes, recordStatus]);
  ...
  return <section className={`artifact-library ${dragging ? "is-dragging" : ""}`} aria-label="Artifact library"
    ...
    {loadError ? <div className="error-notice" role="alert"><p>{loadError}</p><button className="button button-secondary" onClick={() => setRefresh((value) => value + 1)}>Retry loading artifacts</button></div> : <>
      ...
      {page && !page.items.length && <div className="artifact-empty"><h2>{search || workFilter ? "No matching artifacts." : "Your project files belong here."}</h2><p>{search || workFilter ? "Try another metadata search or clear the work filter." : "Keep documents, binaries and working files together with the work that created them."}</p></div>}
