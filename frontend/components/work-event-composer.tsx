...
export default function WorkEventComposer({
  ...
  return <form className="event-composer" onSubmit={(event) => void submit(event)} aria-labelledby={titleId}>
    ...
    <label className="field">Progress text<textarea rows={4} value={body} onChange={(event) => {
    ...
    <div className="event-composer-actions"><button type="submit" className="button button-secondary" disabled={saving || !body.trim()}>{saving ? "Adding…" : "Add progress update"}</button></div>
  </form>;
}
