  ...
  return <>
    ...
    {!isDuplicate && <section className="checkpoint-compose" aria-labelledby="checkpoint-compose-title">
      ...
      <form className="comment-form" onSubmit={(event) => { event.preventDefault(); props.onAppend(); }}>
        ...
        {context.work_item.status === "pending" && <JobReportEditor projectId={context.work_item.project_id} workItemId={context.work_item.id} draft={props.jobReportDraft} onChange={props.onJobReportDraft} disabled={props.checkpointSaving || immutable} />}
...
export function StatusActionButton({
  ...
  return <TaskStatusActionButton
    subject={work}
    actions={availableStatusActions(work.status, readiness, Boolean(work.manual_review_request)).map((action) => ({
      ...action, disabledReason: statusActionDisabledReason(action.value, readiness, reportSettingsReady)
    }))}
    deferred={(readiness.review_status ?? work.status) === "deferred"}
    projects={projects} disabled={disabled} busy={busy} moveDisabled={moveDisabled}
    moving={moving} moveTitle={moveTitle} moveExplanationId={moveExplanationId}
    onAction={(action) => onAction(action, summary)} onMove={onMove} compact={compact}
  />;
}
...
function OpenedPane({ opened, props }: { opened: WorkSummary; props: WorkDetailPaneProps }) {
  ...
  return <>
    <div className="detail-header">
      ...
      {work.manual_review_request && work.status !== "done" && <p className="field-hint">Code review requested by a human operator. It will enter To review when this work is Done.</p>}
