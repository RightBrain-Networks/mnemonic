...
import { decodePhase12Cursor, type ProvenanceCursor } from "@/lib/activity-cursors";
...
function ProvenanceDirection({ projectId, workItemId, direction, refreshSignal, onOpenWork }: {
  ...
  refreshSignal: number; onOpenWork: (workItemId: string) => void;
}) {
  ...
  useEffect(() => {
    ...
    api<unknown>(`/projects/${projectId}/work-items/${workItemId}/report-follow-ups?${params}`, { signal: controller.signal })
      .then((raw) => {
        ...
        if (cursor) {
          const previous = decodePhase12Cursor(cursor, projectId, "work_report_follow_ups") as ProvenanceCursor;
          if (next.as_of_sequence !== previous.upper || next.items.some((item) => BigInt(item.created_sequence) <= BigInt(previous.last))) {
  ...
  useEffect(() => {
    ...
    api<unknown>(`/projects/${projectId}/job-completion-reports/${selectedReport}`, { signal: controller.signal })
      .then((raw) => { if (!controller.signal.aborted) setReport(decodeReportDetail(raw, projectId, selectedReport)); })
    ...
    return () => controller.abort();
  }, [projectId, selectedReport, refreshSignal, reportReload]);
  ...
  useFailedReadRetry({ scope: `originating-report:${projectId}:${selectedReport}`, failed: Boolean(reportError), busy: reportLoading, enabled: Boolean(selectedReport), retry: () => setReportReload((value) => value + 1) });
  return <section>
    ...
    {items.length > 0 && <ul>{items.map((link) => <li key={link.id}>
      <button type="button" className="button button-secondary" onClick={() => setSelectedReport(link.report_id)}>Read originating report</button>
      <p>Report <span className="mono">{link.report_id}</span></p>
      <p>Exact source work <span className="mono">{link.source_work_item_id}</span> <button type="button" className="button button-secondary" onClick={() => onOpenWork(link.source_work_item_id)}>Open original work</button></p>
      <p>Follow-up work <span className="mono">{link.follow_up_work_item_id}</span> <button type="button" className="button button-secondary" onClick={() => onOpenWork(link.follow_up_work_item_id)}>Open follow-up work</button></p>
    </li>)}</ul>}
...
export default function WorkReportProvenance(props: {
  projectId: string; workItemId: string; refreshSignal: number; onOpenWork: (workItemId: string) => void;
}) {
