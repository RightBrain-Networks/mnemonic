...
export default function JobReportContent({ item }: { item: JobReportEnvelope }) {
  ...
  return <div className="job-report-content">
    ...
    <p className="human-report-summary" dir="auto">{report.summary}</p>
    {report.fyi_items.length > 0 && <ul className="human-report-fyis">{report.fyi_items.map((item, index) => <li key={index} dir="auto">{item}</li>)}</ul>}
