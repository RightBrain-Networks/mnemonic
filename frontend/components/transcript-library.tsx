...
function TranscriptDetails({ transcript: initial, onClose }: { transcript: Transcript; onClose: () => void }) {
  ...
  const rows: [string, ReactNode][] = [
    ...
    ["Session type", transcript.kind === "primary" ? "Primary" : "Subagent"], ["Transcript ID", transcript.id],
    ["Work item", <a key="work" href={`/?work=${transcript.work_item_id}`}>{transcript.work_item_id}</a>],
    ["Lease generation", transcript.lease_generation_id], ["Source SHA-256", transcript.sha256 || "Not recorded"], ["Indexed text SHA-256", transcript.text_sha256 || "Not recorded"]
  ];
