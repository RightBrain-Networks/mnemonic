...
The current application/API/MCP/dashboard release is `0.56.0`, Claude plugin
`0.34.0`, and Alembic head `0039_manual_review_requests`. The catalog is exactly
...
Local uploads/replacements use `scripts/upload_artifact.py prepare`, then
`authorize_artifact_upload` with its exact `upload_intent`, then `send --grant-file`.
